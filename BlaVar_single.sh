#!/bin/bash
#OPTIONS
restart=1 #1 to activate / Must use for 'delta'
...
POWER=0.2 #power p where: param propto (Flux_gamma)^p / 1.0 for simple use 		   #example:  for delta POWER= '0.25'
...
cp $route/delta_analysis.py ./delta_analysis.py
cp $route/code_var ./
