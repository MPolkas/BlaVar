...
!     ARRAY DEFINITIONS
     ...
     $         ,yk(npmax), pks(npmax), gk(npmax)    !pk
      ...
      DIMENSION fktime(100000),fakegm(100000)
...
!  sxnd is is a measure of the density in photons
	...
	do k=1,np
	 ...
	 do k=1,ng
      ...
      write (55,1000) t,geextmx !FAKE TIMECURVE
...
C  Output on screen:
	...
	write (6,1000) log10(geextmx), tol
...
!      common/times/ttime,tprev
       ...
       if(time.gt.fktime(1))then
        ...
        geextmx=(fakegm(i+1)-fakegm(i))/dfk*
     &  (time-fktime(i))+fakegm(i)
