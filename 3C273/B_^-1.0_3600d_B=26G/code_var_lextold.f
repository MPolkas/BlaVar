...
!     ARRAY DEFINITIONS
     ...
     $         ,yk(npmax), pks(npmax), gk(npmax)    !pk
      ...
      DIMENSION fktime(100000),fakelext(100000)
...
!     DECLARATION OF SUBROUTINES      
     ...
     $ ap,iprext,ipexp  
      ...
      common/ebb/ygbb,yglbb,tol,nbbmx 
      common/tvv/iprcl 
      common/bet/fpbh
      common/prl/floss
...
!       co-moving isotropic-equivalent proton injection luminosity [erg/s]
        ...
        write(6,*) 'B [G], UB [erg/cm^3], R [cm], input dt [t_cross]'
	write (6,1000) Bfield,UB,Radius,dfk
...
!     PHYSICAL CONSTANTS        
     ...
     $ ae,ielext,ieexp  
  !!!     FAKE TIMECURVE
      DIMENSION fktime(100000),fakelext(100000),extphin(npmax)
      common/fake/ifake
      common/tm/fktime,fakelext,dfk
...
!! PHOTON INJECTION
        call phbbinj(time,x,yg,ng,deltap,extphin,sumphinj)
...
!       photons:
 	yprime(np+ne+1)=extphin(1)-ipsc*1./(1.+tauth*zd(1)/3.)+ 
...
!       photons:
        yprime(np+ne+ng)=extphin(ng)-ipsc*1./(1+tauth*zd(ng)/3.)+   
...
!   PHOTONS
	...
	do n=1,ng
	yprime(np+ne+n)=extphin(n)-ipsc*1./(1.+tauth*zd(n)/3.) +
...
C  Output on screen:
	write (6,1400) iprcl,time,nmx,y(nmx),yprime(nmx),tauth, xdten
	write (6,1000) yglbb,log10(yglbb),tol
...
*************************************************************************
!!    EXTERNAL PHOTON INJECTION (SINGLE BLACK BODY)  /     FAKE TIMECURVE 1
      subroutine phbbinj(time,x,yg,ng,deltax,extphin,sumphinj)
      parameter (npmax=400,ntotal=800)
      implicit real*8(a-h,o-z)
      integer ieexp
!     PHYSICAL CONSTANTS        
     ...
     $ pi=3.14159265,boltz=1.38e-16,astbo=7.56e-15,Bcr=4.4e13)
      ...
      DIMENSION fktime(100000),fakelext(100000)
      DIMENSION ygbb(npmax),x(ng),yg(ng),extphin(ng)
...
!         write(6,*) xlext,i,fktime(i)
	...
	bbnorm=45.*yglbb/pi**4./xbb**4./xnorm/3. !! bc - check the 3 - ec!!           
 	extphin(n)=log(bbnorm*x(n)**2./(exp(x(n)/xbb)-1.))
	sumphinj=sumphinj+deltax*x(j)**2.*exp(extphin(j))*exp(yg(j))*xnorm
309    continue
 3     continue
