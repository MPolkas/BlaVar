...
!     ARRAY DEFINITIONS
     ...
     $         ,extphin(npmax)!,extBB(npmax) !FAKE TIMECURVE
...
301     continue 
        ...
        y(np+ne+ng+1+ne+np+n)= log(exp(yg(n))+exp(ygbb(n)))!noBB coevolve external photon field
3       continue 
	endif
...
!     PHYSICAL CONSTANTS        
	...
	if (x(n).gt.1.) zd(n)=0.
      ...
      !FAKE TIMECURVE
        ygbb(n)=y(np+ne+ng+1+ne+np+n)-log(1.+tauth*zd(n)/3.)  !noBB coevolve external photon field
      enddo
...
!     $  log10(xnorm*exp(ygbb(n))*x(n)**2)
	...
	write (85,1000) log10(x(n)),
     $  log10(xnorm*exp(ygbb(n))*x(n)**2)
	if((yg(n)-ygbb(n)).gt.0.)then
        write (86,1000) log10(x(n)),
     $  log10(xnorm*(exp(yg(n))-exp(ygbb(n)))*x(n)**2)
        else
        write (86,1000) log10(x(n)), -1.0000E+02
        endif
        enddo
...
!         if (x(n).gt.30.*xbb) goto 309 
	 bbnorm=45.*yglbb/pi**4./xbb**4./xnorm/3. !! bc - check the 3 - ec!!           
 	 extphin(n)=bbnorm*x(n)**2./(exp(x(n)/xbb)-1.)/(exp(yg(n)))
! 	 extBB(n)=bbnorm*x(n)**2./(exp(x(n)/xbb)-1.)/(exp(yg(n)))
!         extBB(n)=bbnorm*x(n)**2./(exp(x(n)/xbb)-1.)/(exp(ygbb(n)))
...
!       photons:
 	yprime(np+ne+ng+1+ne+np+1)= -ipsc*1./(1+tauth*zd(1)/3.) +!noBB
...
!     Main Photons:
        yprime(np+ne+1)=yprime(np+ne+ng+1+ne+np+1)+
     $  extphin(1) !FAKE TIMECURVE
...
!        photons:
        yprime(np+ne+ng+1+ne+np+ng)= -ipsc*1./(1+tauth*zd(ng)/3.)+  !noBB
...
!       main photons:
        yprime(np+ne+ng)=yprime(np+ne+ng+1+ne+np+ng)+
     $  extphin(ng) !FAKE TIMECURVE
...
!   PHOTONS
	...
	do n=1,ng
	yprime(np+ne+ng+1+ne+np+n)= -ipsc*1./(1.+tauth*zd(n)/3.)+                                         ! noBB
...
! treatment of external photon field   !FAKE TIMECURVE
        yprime(np+ne+n)=yprime(np+ne+ng+1+ne+np+n)+  !inject photons
     $  extphin(n) !FAKE TIMECURVE
...
!
	...
	yprime(np+ne+ng)=0.
	yprime(np+ne+ng+1+ne+np+ng)=0. !extBB
