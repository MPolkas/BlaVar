...
routef = '/media/Datas/BLAVAR/Var/' #route for folder where observation folders are in # /home/user/Var/ where Var contains folder 3C273
routeLCsim = '/media/Datas/BLAVAR/Lightcurve_Simulation/'  #route for LightcurveSimulation creation; where the my_analysis_v2.py has run
...
monochromVIR, radio, Xrays, gammarays ='no' , 'no', 'no', 'no'  #only if TIMELAPSE on

"""Plot frames in SED with specific time interval"""
sampleframes='no'
dtsmpl=100 # t_cross (check your time - interval for selection)

SED = 'con' #'no', 'on'(overplot), 'con' (contour)  vFv Diagram [CAUTION!]only up to *limSEDs* timepoints overlayed
...
DCFplot='no' #automatically on if DCFcalc on
SFcalc='no'
taubin=1.0
...
cmps={'le':plt.cm.RdPu,'B':plt.cm.GnBu,'delta':plt.cm.YlGn,'lext':plt.cm.YlOrRd,'gmax':plt.cm.PuBu}
nbx=475 #x-histogram
nby=350 #y-histogram / check data-reduction parameter THRESHOLD for setting value
Nlvs=10
toplvl=0.3
...
cn4=open(routed+'code_new4.inp','r')
lines=cn4.readlines()
cn4.close()
...
if SSConly=='no':
        def BB(x,T,lext,Rbb):
            Ivbb=3*x+np.log10(2*me**3*c**4/h**2)- (10**x*me*c**2/kb/T)*np.log10(np.exp(1))+ np.log10(1-np.exp(-10**x*me*c**2/kb/T))
...
if (SED!='no' or TIMELAPSE!='no'):
    ...
    errx=np.mean(errorvFv[v_pts<np.log10(range_xhard[1])][v_pts[v_pts<np.log10(range_xhard[1])]>np.log10(range_xsoft[0])])
    errg=[0.135, 0.1456 , 0.2386] # [oo] # average relevant error for error < norms cases
    if Multicolor=='on':
...
#read multi.log to correct for times of different runs IF INCOMPATIBILITY WITH INPUT RUNTIME ADN Tmax
#if len(names)>1:
#    mlog=open(route+'multi.log','r')
#    tms=mlog.readlines()
#    mlog.close()
#    tms=np.array(tms).astype(np.float64)
...
for name in names:
    ...
    for y in varfile:
        ...
        filevar.append(w)
    varfile.close()
epsfile=np.array(epsfile).astype(np.float64)
Ifile=np.array(Ifile).astype(np.float64)
gammafile=np.array(gammafile).astype(np.float64)
Nfile=np.array(Nfile).astype(np.float64)
filetime=np.array(filetime).astype(np.float64) 
filevar=np.array(filevar).astype(np.float64)
...
print('Runtime calculated from adding up all fakeTC_*.txt files:\t',str(max(filetime*tcross/delta)))
...
for y in fsteady:
            ...
            epsst.append(z)
            Ist.append(w)
epsst=np.array(epsst).astype(np.float64)[-LENst+1::]
Ist=np.array(Ist).astype(np.float64)[-LENst+1::]
...
""" Real Fermi-LAT LC used for that object"""
if realLC=='on' and SED!='no':
        ...
        if name55=='fakeTC.txt':
            rt=rt+filetime[list(filevar).index(max(filevar))]*tcr-rt[rf.index(max(rf))]+addtcross*tcr
        else:
            rt=rt+filetime[list(filevar).index(max(filevar))]-rt[rf.index(max(rf))]+addtcross*tcr
        rf=np.array(rf)
        rf=np.log10(rf)
...
if (SED!='no' or sampleframes=='on') and TIMELAPSE!='on':
        ...
        uplimpks = 0
        if oo==2: uplimpks+=1
        lims=[round(min(v_pts)-0.5),round(max(v_pts)+0.5+uplimpks),\
...
legs=[]
time , tobs, smarts , xsoft , xhard , fermi, gbrnum, Lgamma, Le = np.zeros(nmax+1), np.zeros(nmax+1), np.zeros(nmax+1), np.zeros(nmax+1), np.zeros(nmax+1), np.zeros(nmax+1), np.zeros(nmax+1),np.zeros(nmax+1),np.zeros(nmax+1)
timesB = [] #for contour times
if SMARTSmulti=='on':
...
for nj in range(n1,nmax):
    ...
    if time[nj-n1+1] not in time[0:nj-n1] and time[nj-n1+1]>0:
        ...
        if len(eps85)>1 and len(I85)>1:
            ...
            timecon = np.floor(time[nj-n1+1]/tbin)
            if SED=='con' and timecon not in timesB:
                xa=xsed+np.log10(delta*(me*c**2/h)/(1+zsh)) #observing (SED) units
                xb=xa[xa>min(x85)+0.01]
                xc=xb[xb<max(x85)-0.01]
                seds.write(str(timecon*tbin)+'\n')
                timesB.append(timecon)
            ...
            if SMARTSmulti!='on':
                if min(lnm)>10**min(x85):
                    smarts[nj-n1+1]=intg.trapz(ymodel(lnm)/lnm,lnm)
                else:
            ...
            if min(lnx)>10**min(x85):
                xsoft[nj-n1+1]=intg.trapz(ymodel(lnx)/lnx,lnx)
            else:
                xsoft[nj-n1+1]=xsoft[nj-n1]
                print('Error with soft X-rays at:\t'+str(nj))
            if min(lnxx)>10**min(x85):
                xhard[nj-n1+1]=intg.trapz(ymodel(lnxx)/lnxx,lnxx)
            else:
                xhard[nj-n1+1]=xhard[nj-n1]
                print('Error with hard X-rays at step:\t'+str(nj))
            fermi[nj-n1+1]=intg.trapz(ymodel(lng)/lng,lng)
            #isotropic luminosities NOT boosted
            Lgamma[nj-n1+1]=intg.trapz(ymodel(10**x85)/10**x85,10**x85)*4*np.pi*D**2
            Le[nj-n1+1]=intg.trapz(10**(N89-2*g89)*(10**g89-1),10**g89)*4*np.pi*me*c**3*R/sigmaT
            ...
            if TIMELAPSE=='on':
                    ...
                    plt.plot(x85,y85,'k-',linewidth=1.5,label='t= {0:.2f}'.format(round(tobs[nj-n1+1],2))+' days')
                    if SSConly!='on':
                        faclext=1
                        if namevar=='lext': faclext = fkvar[nj-n1+1]/lext
                        plt.plot(xbb,ybb+np.log10(faclext),'b--',linewidth=0.5,label='BLR')
                    if secondBB=='on':
                ...
                if realhist=='on':
                    ax10.plot(x85,y85,'-',color=onecolor,alpha=0.5,linewidth=0.6-0.1*((nmax-n1)//1000))
                else:
                    plt.figure(1)
                    if onecolor!='no':
                        plt.plot(x85,y85,'-',color=onecolor,alpha=0.5,linewidth=0.6-0.1*((nmax-n1)//1000))
                    else:
            ...
            if TIMELAPSE=='on' and (radio=='on' or monochromVIR=='on' or Xrays=='on' or gammarays=='on'):
              figall=plt.figure(2)
              figall.suptitle(obj+'  t= {0:.2f}'.format(round(tobs[nj-n1+1],2))+' days',fontsize=16)
              if monochromVIR=='on':
                ...
                if Multicolor=='on':
                    for i in range(len(dtpts)):
                        ms , alpha = 5.0 , 1.0
                        if colors[i]=='grey': ms ,alpha = 1.5 , 0.7
                        plt.errorbar(globals()['v_pts'+str(i)],globals()['vFv_pts'+str(i)]-globals()['v_pts'+str(i)],\
                                 yerr=globals()['errorvFv'+str(i)],\
                                 elinewidth=1.5,capsize=2.5,capthick=0.5,ecolor=colors[i],\
                                 fmt=form[i],color=colors[i],ms=ms,fillstyle=fillstyle[i],\
                                 label=legend_pts[i] , alpha = alpha)
              ...
              if Xrays=='on':
                ...
                if Multicolor=='on':
                    for i in range(len(dtpts)):
                        ms , alpha = 5.0 , 1.0
                        if colors[i]=='grey': ms ,alpha = 1.5 , 0.7
                        plt.errorbar(globals()['v_pts'+str(i)]+np.log10(h/me/c**2),globals()['vFv_pts'+str(i)] -  globals()['v_pts'+str(i)]-np.log10(h),\
                                 yerr=globals()['errorvFv'+str(i)],\
                                 elinewidth=1.5,capsize=2.5,capthick=0.5,ecolor=colors[i],\
                                 fmt=form[i],color=colors[i],ms=ms,fillstyle=fillstyle[i],\
                                 label=legend_pts[i] , alpha = alpha)
              ...
              if radio=='on':        
                ...
                if Multicolor=='on':
                    for i in range(len(dtpts)):
                        ms , alpha = 5.0 , 1.0
                        if colors[i]=='grey': ms ,alpha = 1.5 , 0.7
                        plt.errorbar(globals()['v_pts'+str(i)], globals()['vFv_pts'+str(i)]-globals()['v_pts'][i],\
                                 yerr=globals()['errorvFv'+str(i)],\
                                 elinewidth=1.5,capsize=2.5,capthick=0.5,ecolor=colors[i],\
                                 fmt=form[i],color=colors[i],ms=ms,fillstyle=fillstyle[i],\
                                 label=legend_pts[i] , alpha = alpha)
              ...
              if gammarays=='on':
                ...
                if Multicolor=='on':
                    for i in range(len(dtpts)):
                        ms , alpha = 5.0 , 1.0
                        if colors[i]=='grey': ms ,alpha = 1.5 , 0.7
                        plt.errorbar(globals()['v_pts'+str(i)]+np.log10(h/eV/10**9),globals()['vFv_pts'+str(i)]+np.log10(3600/h) - globals()['v_pts'+str(i)],\
                                 yerr=globals()['errorvFv'+str(i)],\
                                 elinewidth=1.5,capsize=2.5,capthick=0.5,ecolor=colors[i],\
                                 fmt=form[i],color=colors[i],ms=ms,fillstyle=fillstyle[i],\
                                 label=legend_pts[i] , alpha = alpha)
...
if addBB=='on' and SSConly=='no':
    ...
    smarts=smarts+noise*(intg.trapz(bbmodel(lnsm)/lnsm,lnsm)+intg.trapz(bb2model(lnsm)/lnsm,lnsm))
    lnx=np.linspace(range_xband[0],range_xband[1],Nint)
    xband=xband+intg.trapz(bbmodel(lnx)/lnx,lnx)+intg.trapz(bb2model(lnx)/lnx,lnx)
    if namevar=='lext':
            smarts=smarts+noise*(intg.trapz(bbmodel(lnsm)/lnsm,lnsm)*fkvar/np.mean(fkvar)+intg.trapz(bb2model(lnsm)/lnsm,lnsm))
            xband=xband+intg.trapz(bbmodel(lnx)/lnx,lnx)*fkvar/np.mean(fkvar)+intg.trapz(bb2model(lnx)/lnx,lnx)
...
filterA=np.array(filterA).astype(np.float64)
filterB=np.array(filterB).astype(np.float64)
ci=np.array(ci).astype(np.float64)
#label color
cilbl='('+cibnda+' - '+cibndb+') '
...
#defaults if tbin = -1
arn=['xsoft','xhard','xband','fermi','smarts','Lgamma','Le','fkvar','fktime']
if CI=='on':
    arn.append('ci')
    arn.append('filterA')
    arn.append('filterB')
...
if SAVE=='on':
    ...
    np.savetxt('LUMS.txt', np.c_[tobs,Lgamma,Le])
    if sampleframes=='on':
        frms.write('STEADY\n')
...
if TIMELAPSE!='on' and (SED!='no' or sampleframes=='on'):
        ...
        # 2-D Histogram --> to be commented out
            ...
            seds.close()
            xseds=np.array(xseds).astype(np.float64)
            yseds=np.array(yseds).astype(np.float64)
...
#            
#            #plotting ICS and SSC components 
#            Bst = route.split('B=')[1].split('G')[0] #steady state magnetic field
#            ssc_steady = open(routed+'steady_SSC_'+Bst+'.85')
#            LENecs, LENssc = 0, 0 
#            for ln in ssc_steady:
#                if float(ln.split("  ")[1])==0. and LENst!=0:
#                   break
#                LENssc+=1
#            epsst, Ist = [ ],  [ ]
...
#                        epsst.append(z)
#                        Ist.append(w)
#            eps=np.array(epsst).astype(np.float64)[-LENst+1::]
#            I=np.array(Ist).astype(np.float64)[-LENst+1::]
#            x_ssc=np.log10(delta*10**eps[I>max(I)-THRESHOLD]*(me*c**2/h)/(1+zsh))  #observing (SED) units
#            y_ssc=np.log10(10**I[I>max(I)-THRESHOLD]*delta**4/D**2*R*me*c**3/sigmaT/3)  #observing (SED) units
#            ssc_model = inpl.interp1d(x_ssc,y_ssc)
#            
#            ax10.plot(x_ssc,y_ssc,'--',color = 'grey',linewidth=0.9,label='SSC')
#            ytemp  =10**ysteady[xsteady<max(x_ssc)]-10**ssc_model(xsteady[xsteady<max(x_ssc)])
#            xtemp  = xsteady[xsteady<max(x_ssc)]
#            ax10.plot(xtemp[ytemp>0.],np.log10(ytemp[ytemp>0.]),'-.',color = 'grey',linewidth=0.9,label='ECS')
#     
        if secondBB=='on':
        ...
        if namevar=='lext':
            xTbb=np.log10(kb*T/me/c**2*np.array([0.15,8.9]))+np.log10(delta*(me*c**2/h)/(1+zsh))
        ...
        if 'on' in SED:
            ...
            ax10.errorbar(np.log10(SMdic[cibndb]),lims[3]-.9,xerr=wbd1,color='red',ms=0.0,capsize=3.5)
            ax10.annotate(cibndb,(np.log10(SMdic[cibndb])-0.25,lims[3]-0.8),fontsize=13)
            ax10.errorbar(np.log10(SMdic[cibnda]),lims[3]-0.9,xerr=wbd2,color='c',ms=0.0,capsize=3.5)
            ax10.annotate(cibnda,(np.log10(SMdic[cibnda])-0.25,lims[3]-0.8),fontsize=13)
            ax10.errorbar(np.mean(np.log10(range_xsoft)),lims[3]-0.9,xerr=wbdx,color='grey',ms=0.0,capsize=3.5)
            ...
            ax10.errorbar(np.mean(np.log10(range_fermi)),lims[3]-0.9,xerr=wbdf,color='blue',ms=0.0,capsize=3.5)
...
#%%"""PLOTTING TIMECURVES"""
    ...
    #    trndB=list(tobsB).index(max(tobsB[tobsB<rndtime+50]))
        ...
        limya24=round(min(min(np.log10(fermi[irnd:trnd])),min(np.log10(smarts[irnd:trnd])),min(np.log10(xband[irnd:trnd])),np.log10(min(fkvar[irnd:trnd])*np.mean(fermi*2)/np.mean(fkvar))),1)-0.05
        limyb24=round(max(max(np.log10(fermi[irnd:trnd])),max(np.log10(smarts[irnd:trnd])),max(np.log10(xband[irnd:trnd])),np.log10(max(fkvar[irnd:trnd])*np.mean(fermi*2)/np.mean(fkvar))),1)+0.05
...
#%% COLORS AND MORE CORRELATIONS
        #FIT POWER LAW
        fit16='on' # 'on' /'no' for powerlaw
        if namevar in ['le', 'B', 'lext']:
            if namevar=='le':
                cpns = fkvarB #rebinned 
                lblcomp = 'le'
            elif namevar=='B':
                cpns = lbB #rebinned
                lblcomp = 'lb'
                ...
                cpns = fkvarB # rebinned
                lblcomp = 'lext'
            fig16,ax16=plt.subplots(num=16)
            lx = np.log10(cpns)
            ly = np.log10(LgammaB)
            if SPOTS=='on':
            ##PLOT MARKERS
                ax16.plot(lx,ly,'.',color='gold',ms=mscorr,zorder=0)
                if CONTOUR=='on':
                    X,Y,Z=surface_density(lx,ly,[])
                    ax16.contour(X,Y,Z,colors=[colcont]*len(lvls),linestyles='dashed',linewidths=2.0,levels=list(np.array(lvls)*np.amax(Z)))
            else:
                ax16.plot(lx,ly,'.','r-', ms=mscorr,zorder=0)
            aa,bb,cc,dd=[ax16.get_xlim()[0],ax16.get_xlim()[1],ax16.get_ylim()[0],ax16.get_ylim()[1]]
            ae,be,ce,de = (1-np.sign(aa)*0.05)*aa, (1+np.sign(bb)*0.05)*bb, (1-np.sign(cc)*0.05)*cc, (1+np.sign(dd)*0.05)*dd
            axlim1, axlim2 =min(ae,ce) , max(be,de)
            ax16.plot(np.linspace(axlim1,axlim2),1*(np.linspace(axlim1,axlim2)-np.mean(lx))+np.mean(ly),'k-')
            ax16.plot(np.linspace(axlim1,axlim2),2*(np.linspace(axlim1,axlim2)-np.mean(lx))+np.mean(ly),'k-.')
            ax16.plot(np.linspace(axlim1,axlim2),-2*(np.linspace(axlim1,axlim2)-np.mean(lx))+np.mean(ly),'k--')
            plt.xlabel(r'$'+ lblcomp+'$',fontsize=corrfont)
            plt.ylabel(r'$log\,L_{\gamma} $',fontsize=corrfont)
            plt.title(obj+r' ${'+lblcomp+'}$ vs $L_{\gamma}$',fontsize=corrfont+2)
            lgs = [r'$\tau_{lag}=0\;$ days', r'$L_\gamma \propto '+lblcomp+'$', r'$L_\gamma \propto {'+lblcomp+'}^2$', r'$L_\gamma \propto {'+lblcomp+'}^2$']
            if fit16=='on':  
                incl=optmz.curve_fit(line, lx-np.mean(lx), ly-np.mean(ly))[0][0]
                ax16.plot(lx,incl*(lx-np.mean(lx))+np.mean(ly),'m-')
                lgs.append(r'$L_{\gamma} \propto '+lblcomp+'^{'+str(round(incl,2))+'}$')
            if SSConly=='on':
                if namevar=='le':
                    ax16.vlines(np.log10(lb/12),ce ,de ,linestyle='--')
                if namevar=='B':
                    ax16.vlines(np.log10(le*12),ce ,de ,linestyle='--')
                lgs.append('lb=12 le')
            ax16.axis([ae,be,ce,de])
            ax16.legend(lgs,fontsize=legsize,markerscale=2)
            ax16.xaxis.set_minor_locator(AutoMinorLocator())
            ax16.yaxis.set_minor_locator(AutoMinorLocator())
            ax16.tick_params(axis='y',labelsize=corrfont-3)
            ax16.tick_params(axis='x',labelsize=corrfont-3)
            plt.tight_layout()
        ...
        if CI=='on':
            ...
            smvci=cibndb
            limflux=18
            smci=10**filterBB[filterBB<limflux] #select x axis from SMARTS bands and make it color
            fig17,ax17=plt.subplots(num=17)
            if SPOTS=='on':
            ##PLOT MARKERS
                #ax17.plot(np.log10(smci)-np.mean(np.log10(smci)),ci-np.mean(ci),'c.',ms=mscorr,zorder=0) #oldci
                ax17.plot(np.log10(smci),ciB[filterBB<limflux],'c.',ms=mscorr,zorder=0)
    
                if CONTOUR=='on':
                    #X,Y,Z=surface_density(np.log10(smci)-np.mean(np.log10(smci)),ci-np.mean(ci),[]) #oldci
                    X,Y,Z=surface_density(np.log10(smci),ciB[filterBB<limflux],lims=[]) #oldci
    ...
    #    
        ...
        if SAVE=='on':
            ...
            if namevar in ['le','B','lext']:
                plt.figure(16).savefig(routeimage+'corr_'+namevar+'Lg-lvar'+spots*'_SP'+titlenums+imtyp,bbox_inches='tight')
            if CI=='on':
    ...
    if DCFcalc=='on':
            ...
            for bnd in range(lnbd):
                ...
                tlags.append(np.array(tlag).astype(np.float64))
                DCFs.append(np.array(DCF).astype(np.float64))
                if bnd==0:
                    bands[-1]=band1
                    bands[0]= band2            
            #PLOTTING
    ...
    if SFcalc=='on':
            ...
            for bnd in range(lnbd):
                ...
                for tau in logtau:
                    ...
                    step=step+1 #count steps
                sflags.append(np.array(sflag).astype(np.float64))
                SFs.append(np.array(SF).astype(np.float64))
    ...
    if 'DCFs.txt' in nmlist:
        ...
        tlfile = open(route+'tlags.txt','w') #write calculated timelags
        ...
        if DCFplot =='on':
            ...
            for bnd in range(lnbd):
                ...
                print('\nDCF created for timelag of '+bands[bnd]+' over '+bands[bnd-1]+\
                 '\nMost strongly detected timelag (peak) at:\t'+str(tlag_max)+' days'+\
                 '\nMaximum with extrapolation method:\t'+str(tlag_extra)+' days')
                tlfile.write('\nDCF created for timelag of '+bands[bnd]+' over '+bands[bnd-1]+\
                 '\nMost strongly detected timelag (peak) at:\t'+str(tlag_max)+' days'+\
                 '\nMaximum with extrapolation method:\t'+str(tlag_extra)+' days')
            if SAVE=='on':  fig25.savefig(routeimage+'tlags_'+namevar+titlenums+imtyp,bbox_inches='tight')
            tlfile.close()
