...
!       protons:
        yprime(1)=extprinxp(1)-iesc*bpresc+syncpr(1)*isyn
!       electrons:
...
!       photons:
     ...
     $	isyn*(syncph(1) + syncprph(1))+ issa*sst(1) +
...
!        photons:
     ...
     $	isyn*(syncph(ng) +syncprph(ng)) +issa*sst(ng) +
...
!! PROTONS + NEUTRONS
	do n=2,np-1!    
! protons	
	yprime(n)=extprinxp(n)-iesc*bpresc+syncpr(n)+
     $	ipair*prloee(n)+ipion*protpg(n)
! neutrons     
...
!   PHOTONS
	...
	yprime(np+ne+n)=-ipsc*1./(1.+tauth*zd(n)/3.) +
     $	isyn*(syncph(n) +syncprph(n)) +issa*sst(n) +
...
C  Output on screen:
	...
	write (6,1000) log10(exlumel), log10(exlumpr) 
...
! What's left of muons for synchrotron				!m
	...
	j=int(1+(log(q*ge(i)**2./206.8)-log(x(1)))/deltax)
...
! What's left of pions for synchrotron				!pk
	...
	j=int(1+(log(q*ge(i)**2./274.)-log(x(1)))/deltax)
		if (j.ge.1.AND.j.LE.ng) then
...
! What's left of kaons for synchrotron				!pk
	...
	j=int(1+(log(q*ge(i)**2./966.7)-log(x(1)))/deltax)
		if (j.ge.1.AND.j.LE.ng) then
