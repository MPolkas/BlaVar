...
!     ARRAY DEFINITIONS
     ...
     $         ,yk(npmax), pks(npmax), gk(npmax)    !pk
      ...
      DIMENSION fktime(10000000),fakele(10000000)
...
!  sxnd is is a measure of the density in photons
	...
	do k=1,np
	 ...
	 do k=1,ng
      ...
      write (55,1000) t,exlumel !FAKE TIMECURVE
...
C  Output on screen:
	...
	write (6,1000) log10(exlumel), tol
...
!      timeb=ttime
!      do ind=1,ifake
! 	if ((ttime.ge.fktime(ind)).and.(timea.ge.timeb)) then
!        if (ttime.ge.fktime(ind)) then
!         exlumel=exlumel+(fakele(ind+1)-fakele(ind))
!     $   /dfk*(timea-timeb)
!	else
!		goto 791 
!	endif
...
!791   continue
!      ttime=timea
!      write(6,*) exlumel,ind,fktime(ind)

       if(time.gt.fktime(1))then
        do i=1,ifake
        if(time.ge.fktime(i).and.time.lt.fktime(i+1))then
        dfk=fktime(i+1)-fktime(i)
        xlrint=(log10(fakele(i+1))-log10(fakele(i)))/dfk*
     &  (time-fktime(i))+log10(fakele(i))
!         write(6,*) xlrint,i,fktime(i)
        endif 
        enddo 
        exlumel=10.**xlrint
!        write(55,1000)  time , xlrint 
        endif
