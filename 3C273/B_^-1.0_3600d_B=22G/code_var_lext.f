...
!     ARRAY DEFINITIONS
     ...
     $         ,ygamma(ntotal),ygbb(npmax),extphin(npmax) !FAKE TIMECURVE
     ...
     $         ,yk(npmax), pks(npmax), gk(npmax)    !pk
      ...
      DIMENSION fktime(100000),fakelext(100000) !FAKE TIMECURVE
...
!     DECLARATION OF SUBROUTINES      
     ...
     $ ap,iprext,ipexp  
      ...
      common/ebb/ygbb,yglbb,xbb,nbbmx
      common/tvv/iprcl 
      common/bet/fpbh
      common/prl/floss
     ...
     $ Rpg07,Rpg08,Rpg09,Rpg10,ppis,pks
      ...
      common/fake/tol,ifake       !FAKE TIMECURVE
      common/tm/fktime,fakelext,dfk !FAKE TIMECURVE
...
!     APPEND TO FILES INSTEAD OF RESETING / FAKE TIMECURVE      
     ...
     $ Access = 'append',Status='old')
      if (ireadin.gt.0) open(unit=86,file='fort.86',
     $ Access = 'append',Status='old')                !extBB
...
300     continue
	xmax=x(ng)
        ntot=np+ne+ng+1+ne+np+ng	!nt!N !extBB
...
301     continue  
	y(np+ne+n)=yg(n)
      ...
      !FAKE TIMECURVE
        y(np+ne+ng+1+ne+np+n)=log(exp(ygbb(n))) !extBB coevolve external photon field
3       continue 
	endif
...
!       y(np+ne+n)=log(exp(yg(n))+ exp(ygbb(n)))
!      !FAKE TIMECURVE
!       y(np+ne+ng+1+ne+np+n)=ygbb(n)  !extBB coevolve external photon field
!5      continue
!       endif
...
!     PHYSICAL CONSTANTS        
     ...
     $	       ,yn(npmax),ynt(npmax) !N!nt
     $         ,ygbb(npmax),ygdiff(npmax),yg2(npmax) !FAKE TIMECURVE
     $         ,xnbb(npmax),ygnbb(npmax)      
      common/param/bplus,tb,qb,gpmax,
     $  deltap,deltax,np,ne,ntot,ng
      common/flags/ipair,isyn,icompt,igg,issa,iesc,
     $ ipsc,ipion,ianni,ikn 
    ...
    !  xnbb1=0.1*xbb
      ...
      !FAKE TIMECURVE
       ygbb(n)=y(np+ne+ng+1+ne+np+n)-log(1.+tauth*zd(n)/3.)  !extBB coevolve external photon field
       ygdiff(n)=log(exp(yg(n))-exp(ygbb(n)))		      !interpolated
!       if(((x(n).le.xnbb1).or.(x(n).ge.xnbb2)).and.
!     $  (x(n).le.1.).and.(x(n).ge.1.e-10)) then
!        xnbb(i)=log(x(n))
!        ygnbb(i)=log(exp(yg(n))-exp(ygbb(n)))
!        i=i+1
!       endif
      enddo
!FAKE TIMECURVE
!START INTERPOLATE
 ...
 !     ypn=0.0
 !     nnbb=i
 !     call spline(xnbb,ygnbb,nnbb,yp1,ypn,yg2)
 !     i=1
 !     do n=1,ng
 !     if((x(n).ge.xnbb1).and.(x(n).le.xnbb2))then!.and.
 !     $(x(n).le.1.).and.(x(n).ge.1.e-6)) then 
 !     call splint(xnbb, ygnbb, yg2, nnbb,log(x(n)),ygtemp)
 !     ygdiff(n)=ygtemp
 !     ygdiff(n)=-30.0
 !    endif
 !     enddo
...
!        if((exp(yg(n))-exp(ygbb(n))).gt.0.)then
!	write (85,1000) log10(x(n)),
!     $  log10(xnorm*(exp(yg(n))-exp(ygbb(n)))*x(n)**2)
!        else
!	write (85,1000) log10(x(n)), -100.000
!        endif
!        write (86,1000) log10(x(n)),log10(xnorm*exp(yg(n))*x(n)**2),
!     $  log10(xnorm*exp(ygbb(n))*x(n)**2)
	if(exp(ygbb(n)).gt.0.)then
	write (71,1000) log10(x(n)),log10(xnorm*exp(ygbb(n))*x(n)**2)
	else
    	write (71,1000) log10(x(n)),log10(xnorm*(exp(yg(n)))*x(n)**2)
	endif
	write (81,1000) log10(x(n)),log10(xnorm*exp(yg(n))*x(n)**2)
        if((x(n).ge.xnbb1).and.(x(n).le.xnbb2))then
        write (85,1000) log10(x(n)), -20.0
        else
	write (85,1000) log10(x(n)),
     $  log10(xnorm*exp(ygdiff(n))*x(n)**2) !ygdiff interpolated area in the area where yg<ygbb FAKE TIMECURVE
        endif
        write (86,1000) log10(x(n)),
     $  log10(xnorm*exp(ygbb(n))*x(n)**2)
        enddo
...
!! ELECTRON INJECTION
        call elinj(time,ge,ye,ne,deltap,extelin,sumelinj)        
...
!  START FAKE TIMECURVE PART 1
        if (time.ge.fktime(1))then
         do i=1,ifake
         if(time.ge.fktime(i).and.time.lt.fktime(i+1))then
         dfk=fktime(i+1)-fktime(i)
         xlext=(log10(fakelext(i+1))-log10(fakelext(i)))/dfk*
     &   (time-fktime(i))+log10(fakelext(i))
!         write(6,*) xlext,i,fktime(i)
         yglbb=10.**xlext
         endif 
         enddo 
        endif

       bbnorm=45.*yglbb/pi**4./xbb**4./xnorm/3.
       do n=1,ng-1
       extphin(n)=0.
       if (x(n).le.30.*xbb)then
 	 extphin(n)=bbnorm*x(n)**2./(exp(x(n)/xbb)-1.)
!        write (6,*) x(n), extphin(n), yg(n), ygbb(n), bbnorm
       endif
       enddo

! END FAKE TIMECURVE PART 1
...
!       photons:
 	yprime(np+ne+1)=extphin(1)/exp(yg(1))- !inject photons
     $  ipsc*1./(1.+tauth*zd(1)/3.)+ 
...
!       extBB:
        yprime(np+ne+ng+1+ne+np+1)=extphin(1)/exp(ygbb(1))-
     $  ipsc*1./(1+tauth*zd(1)/3.)  !FAKE TIMECURVE
...
!        photons:
        yprime(np+ne+ng)=extphin(ng)/exp(yg(ng))-
     $  ipsc*1./(1+tauth*zd(ng)/3.)+   !inject photons
...
!       extBB:
        yprime(np+ne+ng+1+ne+np+ng)=extphin(ng)/exp(ygbb(ng))-
     $  ipsc*1./(1+tauth*zd(ng)/3.)  !FAKE TIMECURVE
...
!	write (86,1000) log10(ge(n)),log10(ge(n)**2.*exp(ynt(n))*x(n)) !86 extBB output 
!	write (83,1000) log10(ge(n)),log10(ge(n)**2.*exp(ynt(n))*tne(n)) 
...
!   PHOTONS
	...
	do n=1,ng
	yprime(np+ne+n)=extphin(n)/exp(yg(n))-
     $  ipsc*1./(1.+tauth*zd(n)/3.)+ !inject photons
...
! treatment of external photon field   !FAKE TIMECURVE
!       extBB:
        yprime(np+ne+ng+1+ne+np+n)=extphin(n)/exp(ygbb(n))-
     $  ipsc*1./(1.+tauth*zd(n)/3.)  !FAKE TIMECURVE
	if (y(np+ne+ng+1+ne+np+n).lt.-200.) yprime(np+ne+ng+1+ne+np+n)=0.	!extBB
...
!
	...
	yprime(np+ne+ng)=0.
	yprime(np+ne+ng+1+np+ne+ng)=0. !extBB
...
!!    ELECTRON INJECTION   
      subroutine elinj(time,ge,ye,ne,deltap,extelin,sumelinj)
      parameter (npmax=400,ntotal=800)
      implicit real*8(a-h,o-z)
      integer ieexp
!     PHYSICAL CONSTANTS        
...
*************************************************************************
 !FAKE TIMECURVE INTERPOLATION SPLINE
************************************************************************
...
!       i.e., yi = f(xi), with x1 < x2 < ... < xN , and given values yp1 and ypn for the first derivative of the 
!       interpolating function at points 1 and n, !!  respectively, this routine returns an array y2(1:n) of
...
!	if (yp1.gt.9.9e29) then !The lower boundary condition is set either to be
!	 y2(1)=0. 		!“natural”
!	 u(1)=0.
...
!	endif
        ...
        do i=2,n-1 !This is the decomposition loop of the tridiagonal
        	      !algorithm. y2 and u are used for temporary storage of the decomposed factors.
...
!	if (ypn.gt.9.9e29) then !The upper boundary condition is set either to be
!	 qn=0.                   !“natural”
!	 un=0.
!	else                         !or else to have a specified first derivative.
        qn=0.5
...
!	and given a value of x, this routine returns a cubic-spline interpolated value y.
	...
	b=(x-xa(klo))/h
	y=a*ya(klo)+b*ya(khi)+1
     *         /6.*((a**3-a)*y2a(klo)+(b**3-b)*y2a(khi))*(h**2)
	return
	end
