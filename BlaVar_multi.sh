#!/bin/bash
#OPTIONS
#this is an expansion after run_var.sh. Works for le, B, lext and gmax. 
#You must have generated the fake Fermi-LAT LC in the relevant folder.

restart=1 #1 to activate,  #have to generate LC first, try run_var.sh
steady=1 #1: new steady state
route="/media/Datas/BLAVAR/Var"
obj='3C273' #no '-' or '_' or '/', raw letters and numbers
lendays=1800 #Desired time interval (days) for reproducing lightcurves
namevar='B' #Variable to be varying
POWER=-1.0 #power p where: param propto (Flux_gamma)^p / 1.0 for simple use 		   #example:  for delta POWER= '0.25'
...
remT=$((${remT%.*}+1))
runnum=0
rm $route/Lightcurve_Simulation/$obj/multi.log
fi
...
cp $route/fakeTC* ./
cp $route/Results_analysis.py ./Results_analysis.py
cp $route/code_var ./
